...
import database.festivaldatabase.FestivalEntry;
import database.festivaldatabase.FestivalScreening;
import database.festivaldatabase.FestivalScreeningId;
...
public class Movies17 extends AbstractMovies {
    ...
    protected static boolean isExploreZone(FilmTitle movie) {
        ...
        for (FestivalEntry entry : entries) {
            if (entry.getId().getYear() == YEAR && entry.getExplorezone() == 1) return true;
        }
        return false;
    }
    ...
    protected static Paragraph getScreening(FestivalScreening screening, Font font) {
        ...
        if (screening.getPress() == 1) p.add(new Chunk(" (press only)"));
        return p;
    }
}
