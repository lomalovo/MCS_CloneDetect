...
public class CDocumentReconstructor {
    ...
    private static final int getSecurityFlags(final Map properties) {
        int securityType = 0;
        securityType = "true".equals(properties.get(IHtmlToPdfTransformer.PDF_ALLOW_PRINTING)) ? (securityType | PdfWriter.AllowPrinting) : securityType;
        securityType = "true".equals(properties.get(IHtmlToPdfTransformer.PDF_ALLOW_MODIFY_CONTENTS)) ? (securityType | PdfWriter.AllowModifyContents) : securityType;
        securityType = "true".equals(properties.get(IHtmlToPdfTransformer.PDF_ALLOW_COPY)) ? (securityType | PdfWriter.AllowCopy) : securityType;
        securityType = "true".equals(properties.get(IHtmlToPdfTransformer.PDF_ALLOW_MODIFT_ANNOTATIONS)) ? (securityType | PdfWriter.AllowModifyAnnotations) : securityType;
        securityType = "true".equals(properties.get(IHtmlToPdfTransformer.PDF_ALLOW_FILLIN)) ? (securityType | PdfWriter.AllowFillIn) : securityType;
        securityType = "true".equals(properties.get(IHtmlToPdfTransformer.PDF_ALLOW_SCREEN_READERS)) ? (securityType | PdfWriter.AllowScreenReaders) : securityType;
        securityType = "true".equals(properties.get(IHtmlToPdfTransformer.PDF_ALLOW_ASSEMBLY)) ? (securityType | PdfWriter.AllowAssembly) : securityType;
        securityType = "true".equals(properties.get(IHtmlToPdfTransformer.PDF_ALLOW_DEGRADED_PRINTING)) ? (securityType | PdfWriter.AllowDegradedPrinting) : securityType;
        return securityType;
    }
